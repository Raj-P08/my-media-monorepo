...
export const MediaProvider: React.FC<MediaProviderProps> = ({
  ...
  const mediaClient = useMemo(() => {
    ...
    if (!apiKey) {
      throw new Error('[MediaProvider Native] Either an `apiKey` string or a `client` instance must be provided.');
    }
...
export const useMediaClient = (): MediaCoreClient => {
  ...
  if (!client) {
    throw new Error('[useMediaClient Native] Must be used within a <MediaProvider>.');
  }
  return client;
};
